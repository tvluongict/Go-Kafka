...
func InitProducer() {
	kafkaWriteLogger = log.New(os.Stdout, "[PAYMENT] Kafka writer: ", 0)
...
func InitConsumer() {
	kafkaReaderLogger = log.New(os.Stdout, "[PAYMENT] Kafka reader: ", 0)
...
func Consume(ctx context.Context) {
	for {
		...
		fmt.Println("[PAYMENT] Received: ", string(msg.Value))
	}
}
